...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "جهت ورود به گروه مهندسی کامپیوتر لطفاً مراحل احراز هویت را انجام دهید:\n"
        "1. شماره تلفن خود را ارسال کنید.\n"
        "2. عکس پروفایل آموزشیار خود را بفرستید (از مسیر ثبت نام دروس دانشجو > مشاهده وضعیت)."
    )
    return PHONE
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if phone_number.startswith("+98") and len(phone_number) == 13:
        ...
        await update.message.reply_text("شماره تلفن معتبر نیست. لطفاً شماره‌ای با کد کشور +98 ارسال کنید.")
        return PHONE
